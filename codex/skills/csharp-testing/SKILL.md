...
| **FluentAssertions 7.x** | Readable assertion syntax — **pin to `7.*`, never `8.0+`** |
| **NSubstitute** or **Moq** | Mocking dependencies |
| **Testcontainers** | Real infrastructure in integration tests |
| **WebApplicationFactory** | ASP.NET Core integration tests |
| **Bogus** | Realistic test data generation |

> **License note**: FluentAssertions moved to a paid commercial license starting at v8.0 (Xceed partnership, Jan 2025) — a paid per-developer license is required for commercial use of v8+. Version 7.x remains free forever under Apache 2.0. Per the paid-packages rule in the global `AGENTS.md`, always add `FluentAssertions` pinned to the `7.*` range in the `.csproj` (e.g. `<PackageReference Include="FluentAssertions" Version="7.*" />`), never let it float to 8+. If a known issue in 7.x blocks something, use a free alternative (e.g. **Shouldly**, MIT-licensed) instead of paying.
